...
def main():
    ...
    parser.add_argument('--lfwPairs', type=str,
                        default=os.path.expanduser(
                            "/home/ubuntu/hemaoliang/open_github/openface/data/lfw/pairs.txt"),
...
def checkPairsHaveEmbed(pair, embeddings):
    if len(pair) == 3:
        name1 = "{}_{}".format(pair[0], pair[1].zfill(4))
        name2 = "{}_{}".format(pair[0], pair[2].zfill(4))
        actual_same = True
    ...
    with open(fname, "w") as f:
        ...
        for threshold in thresholds:
            ...
            for pair in pairsTest:
                if  not checkPairsHaveEmbed(pair, embeddings):
                     continue
...
def plotVerifyExp(workDir, tag):
    ...
    openbrData = pd.read_csv("../evaluation/comparisons/openbr.v1.1.0.DET.csv")
    ...
    humanData = pd.read_table(
        "../evaluation/comparisons/kumar_human_crop.txt", header=None, sep=' ')
    humanPlot, = plt.plot(humanData[1], humanData[0])
    humanAUC = getAUC(humanData[1], humanData[0])

    deepfaceData = pd.read_table(
        "../evaluation/comparisons/deepface_ensemble.txt", header=None, sep=' ')
    ...
    eigData = pd.read_table(
        "../evaluation/comparisons/eigenfaces-original-roc.txt", header=None, sep=' ')
